# accepts number of teams as argument
numberTeams=$1
numberTeams="${numberTeams:-30}"
...
# echo "$WORDS" | tr ' ' '\n' | awk '{ print length, $0 }' | sort -n | cut -d" " -f2 | uniq | head -n ${numberTeams} | sort | xargs

# test our data
for i in {0..${numberTeams}}; do
  team=${teamname[$i]}
  printf "%-15s" "$team"
